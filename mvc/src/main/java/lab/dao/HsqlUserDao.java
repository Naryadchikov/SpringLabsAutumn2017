...
public class HsqlUserDao extends JdbcDaoSupport implements UserDao {
	...
	public void insert(User user) {
		...
		if (user != null) {
			...
			this.getJdbcTemplate().update("insert into user (firstname, lastname) values (?, ?)", user.getFirstName(), user.getLastName());
		} else {
	...
	public User select(int id) {
		...
		if (id > 0) {
			user = this.getJdbcTemplate().queryForObject(
					"select id, firstname, lastname from user where id = ?", new Object[] { id }, new UserMapper());
		}
	...
	public List<User> selectAll() {
		return this.getJdbcTemplate().query("select id, firstname, lastname from user", new UserMapper());
	}
